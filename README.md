...
- **智能体市场**：选择官方 Agent（智能客服 / 电商竞品分析）；运营可上下架
- **电商竞品分析**：多平台自然语言竞品搜索、采集、AI 评论摘要、CSV 导出，结果写入 `data/competitor.db`
- **知识库**：FAQ 增删改、导入（JSON / CSV / TXT / Excel）、重建向量库
- **运营**：敏感词、话术配置、用户管理（JWT：`user` / `ops`）
- **可选**：企业微信回调；飞书事件回调 + 任务中心「我负责的」查询（需用户 OAuth）
- **前端**：对话页、智能体市场、FAQ / 敏感词 / 话术 / 用户管理 / 渠道配置（`web/`）
...
```
├── main.py              # API 入口
├── src/                 # 后端（聊天、RAG、鉴权、FAQ、智能体市场等）
├── mp_agent/            # 电商竞品分析 Agent（迁入适配）
├── config/              # mp_agent 运行时配置（SQLite / DashScope）
├── web/                 # Vue 3 运营 / 对话前端
├── data/
│   ├── faq/             # FAQ 示例与说明（可入库）
│   ├── bot_config.json  # 话术等配置
│   ├── competitor.db    # 竞品分析结果（运行时生成）
│   ├── agents.db        # 智能体市场目录（运行时生成）
│   └── *.db             # 其余运行时 SQLite（本地生成，不入库）
├── .env.example         # 环境变量模板
└── docs/                # 方案与开发笔记
```

运行时数据库（会话、FAQ、敏感词、鉴权、对话日志、竞品、智能体市场等）会写在 `data/*.db`，已由 `.gitignore` 忽略，**请勿提交真实业务数据**。

部分竞品平台采集依赖可选组件：`playwright` / `apify-client` / FlareSolverr；未安装时对应平台任务会报错提示，不影响智能客服与市场浏览。
